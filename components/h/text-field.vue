...
<template>
  <div :class="props.class" class="w-full group">
    <label
      :class="[props.class, props.disabled ? '' : 'group-hover:font-bold']"
      class="text-neutral400 font-semibold mb-1 text-sm"
      :for="name"
    >
      {{ label && $t(label) }}
    </label>
    ...
    >
      ...
      >
        <Icon
          ...
          class="h-5 w-5 mr-1 text-neutral400"
        />
        <p
          v-if="props.leadingText && !props.leadingIcon"
          class="font-semibold text-sm w-10 text-neutral400"
          :class="[
        ...
        <div
          :class="[props.disabled ? '' : 'group-hover:bg-primary']"
          class="w-[1px] h-[60%] bg-neutral400 self-end my-auto transition-all ease-in-out duration-300"
        ></div>
      </div>
      ...
      <input
        ...
        :class="[
          ...
          props.disabled
            ? 'cursor-not-allowed'
            : 'focus:border-primary focus:outline-none focus:shadow-outline  hover:border-primary',
