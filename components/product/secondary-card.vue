<template>
  <nuxt-link
    ...
    class="group rounded overflow-hidden transition-all duration-300 hover:shadow-[0px_0px_24px_8px_rgba(120,120,120,0.2)] p-1 bg-light400 max-w-xs"
  >
    ...
    <hr class="m-1" />
    <div class="px-3 py-1 text-neutral200">
      ...
      <div class="flex items-end justify-end">
        <div v-if="score" class="flex justify-between items-end w-full">
          <div class="flex gap-1 items-center py-1 text-neutral300 text-md">
            <p>Score :</p>
            <svg
              width="18"
              height="16"
          ...
          <div class="w-fit flex justify-end items-end flex-col">
            <p class="text-primary text-lg font-extrabold">{{ price }} ETB</p>
            <div
              class="px-3 my-2 w-fit text-xs font-medium bg-primary text-light300 rounded-full"
            >
              {{ status }}
            </div>
          </div>
        </div>
        <div v-else class="w-fit flex justify-end items-end flex-col">
          <p class="text-primary text-lg font-extrabold">{{ price }} ETB</p>
          <div
            v-if="status"
            class="px-3 my-2 w-fit text-xs font-medium bg-primary text-light300 rounded-full"
