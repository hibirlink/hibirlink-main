...
const variable = computed(() => {
  return {
    ...
    where: {
      ...
      product_discount: {
        rate: { _gte: 0 },
        start_date: { _lte: new Date().toLocaleDateString() },
        end_date: { _gte: new Date().toLocaleDateString() },
      },

      ...(selected_category.value
        ? {
            physical_product_sub_category: {
              category_id: { _eq: selected_category.value },
            },
          }
        : {}),
...
watch(
  [rate, price, location, status, selected_category],
  ([newRate, newPrice, newLocation, newStatus, cat]) => {
    offset.value = 0;
  }
);

const sub_category = computed(() => {
  return store.getPhysicalProductSubCategory(locale.value);
});
